...
func Init() {
	...
	fileConfig := &lumberjack.Logger{
		Filename:   viper.GetString("LOG_PATH"),    // 日志文件名
		MaxSize:    viper.GetInt("LOG_MAXSIZE"),    // 日志文件大小
		MaxAge:     viper.GetInt("LOG_MAXAGE"),     // 最长保存天数
		MaxBackups: viper.GetInt("LOG_MAXBACKUPS"), // 最多备份几个
		LocalTime:  viper.GetBool("LOG_LOCALTIME"), // 日志时间戳
		Compress:   viper.GetBool("LOG_COMPRESS"),  // 是否压缩文件，使用gzip
	}
	encoder := zap.NewProductionEncoderConfig()
	encoder.EncodeTime = func(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
		enc.AppendString(t.Format("2006-01-02 15:04:05.000000"))
	}
	if viper.GetBool("LOG_CONSOLE_STDOUT") {
		syncWriters = append(syncWriters, zapcore.AddSync(os.Stdout))
	}
	if viper.GetBool("LOG_FILE_STDOUT") {
